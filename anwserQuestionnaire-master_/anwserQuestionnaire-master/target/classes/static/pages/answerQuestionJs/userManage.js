...
function nihao(){
    ...
    commonAjaxPost(true, "/admin/addUserInfoList", da, function (result) {
        //console.log(result)
        if (result.code == "666") {
...
function TableInit() {
    ...
    oTableInit.Init = function () {
        $('#userTable').bootstrapTable({
            url: '/admin/queryUserList',         //请求后台的URL（*）
            method: 'POST',                      //请求方式（*）
                ...
                if(res.code == "666"){
                    ...
                    if (userInfo.length) {
                        for (var i = 0; i < userInfo.length; i++) {
                            ...
                            dataNewObj.startTime = userInfo[i].start_time.substring(0,userInfo[i].start_time.length-9).replace("T"," ");
                            dataNewObj.endTime = userInfo[i].stop_time.substring(0,userInfo[i].start_time.length-9).replace("T"," ");
